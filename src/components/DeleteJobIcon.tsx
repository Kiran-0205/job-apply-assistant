  ...
  return (
    <>
      ...
      <button
        ...
        title="Delete job"
        className={`absolute -top-2 -right-2 z-10 w-6 h-6 flex items-center justify-center rounded-full border border-stone-200 bg-white text-stone-400 shadow-sm hover:text-rose-500 hover:border-rose-300 transition-all text-sm leading-none
      ...
      >
        ...
        <button
          ...
          disabled={loading}
          className="text-[11px] font-semibold text-white bg-rose-500 hover:bg-rose-600 disabled:opacity-50 rounded-full px-2 py-0.5 transition-colors"
        >
          {loading ? "…" : "Yes"}
        </button>
        <button
          onClick={(e) => { e.stopPropagation(); setConfirming(false); }}
          className="text-[11px] font-medium text-stone-400 hover:text-stone-600 rounded-full px-1.5 py-0.5 transition-colors"
        >
          No
        </button>
      </div>
